...
class InputSchema(BaseModel):
    ...
    tag_filter: Optional[str] = Field(
        title='Tag Filter',
        description='Filter metrics that have been submitted with the given tags. Supports boolean and wildcard expressions.Cannot be combined with other filters.')
...
def datadog_list_active_metrics(handle,
                                from_time: int,
                                tag_filter: str = "") -> Dict:
    """datadog_list_active_metrics get the list of actively reporting metrics from a given time until now.
