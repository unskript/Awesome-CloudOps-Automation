...
def git_top_dir() -> str:
    ...
    """
    run_output = run(["git", "rev-parse", "--show-toplevel"],
                     capture_output=True)
...
def check_action_by_connector_names(connector: str = '') -> bool:
    ...
    if top_dir not in ('', None):
        dirs_under_connector = os.listdir(
            os.path.join(top_dir, connector, 'legos'))
...
def check_dir_contents(_dir: str, ret_val) -> bool:
    ...
    try:
        ...
        if d.get('action_entry_function') != os.path.basename(_dir):
            print(
                f"ERROR: ENTRY FUNCTION IN {jsonfile} is Wrong Expecting: {os.path.basename(_dir)} Has: {d.get('action_entry_function')}")
            ret_val[_dir] = False
            return None
...
def main():
    ...
    for connector in all_connectors:
        ...
        result.append(check_action_by_connector_names(
            os.path.dirname(connector)))

    for r in result:
        if r is False:
            print("ERROR: Check Failed. Please note the Validation process checks -")
            print(
                "ERROR:     1. Lego Directory name should match python and json file name ")
            print(
                "ERROR:     2. Action Entry function should be the same as Lego directory name")
            print(
                "ERROR:     3. A __init__.py File should exist for every Lego directory")
