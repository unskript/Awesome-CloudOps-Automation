...
class InputSchema(BaseModel):
    threshold_days: int = Field(
        ...
        description='(in days) The threshold to check the IAM user password older than the threshold.')
...
def aws_list_users_with_old_passwords(handle, threshold_days: int = 120) -> List:
    """aws_list_users_with_old_passwords lists all the IAM users with old passwords.
        ...
        :param threshold_days: (in days) The threshold to check the IAM user password older than the threshold.
    ...
    for user in response:
        try:
            ...
            if 'CreateDate' in login_profile['LoginProfile']:
                password_last_changed = parse(str(login_profile['LoginProfile']['CreateDate'])).replace(tzinfo=timezone.utc)
