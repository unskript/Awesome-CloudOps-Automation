...
def configure_credential(creds_dict: dict):
    ...
    if not creds_dict:
        ...
        return 
    if creds_dict.get('enable') == False:
        print("WARNING: No credential programmed, as Enable flag is set to false")
        return 
    
    creds_cmd = []
    for cred in creds_dict.keys():
        if cred.lower() == 'enable':
            continue
...
def main():
    ...
    to parse the hooks.yaml file and program credential and schedule as configured
    """
    retval = parse_hook_yaml('./hooks.yaml')
