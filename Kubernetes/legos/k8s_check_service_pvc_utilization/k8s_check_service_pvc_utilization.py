...
def k8s_check_service_pvc_utilization(handle, service_name: str, namespace: str, threshold: int=80) -> Tuple:
    ...
    """
    # 1. Get the labels associated with the service
    ...
    labels_str = response.stdout.strip().replace("'", "")
    ...
    # 2. Identify a running pod that is utilizing the target PVC using the fetched labels
    ...
    # 3. Get the PVC name mounted in the identified Pod
    get_pvc_name_command = f"kubectl get pod {pod_name} -n {namespace} -o=jsonpath='{{.spec.volumes[?(@.persistentVolumeClaim)].persistentVolumeClaim.claimName}}'"
    response = handle.run_native_cmd(get_pvc_name_command)
    if not response or response.stderr:
        raise ApiException(f"Error while executing command ({get_pvc_name_command}): {response.stderr if response else 'empty response'}")
    pvc_name = response.stdout.strip()
    print(f"PVC associated with pod {pod_name} is: {pvc_name}")
    ...
    print(f"Total PVC capacity: {total_capacity_gib:.2f}Gi")
    print(f"Actual used space in PVC: {used_space_gib:.2f}Gi")
    print(f"Percentage of used space: {used_percentage:.2f}%")
    print(f"Remaining space percentage: {remaining_percentage:.2f}%")

    if remaining_percentage < threshold:
        return (False, [{"pvc_name": pvc_name, "namespace": namespace, "used": f"{used_space_gib:.2f}Gi", "capacity": f"{total_capacity_gib:.2f}Gi"}])
